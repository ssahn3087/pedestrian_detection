...
def simple_distance_loss(self, triplet_features):
    """
    ...
    pos_dist = (torch.sqrt((anchor - positive) ** 2)).sum(0)
    neg_dist = (torch.sqrt((anchor - negative) ** 2)).sum(0)
    ...
    if triplet_features.size(0) > 3:
        ...
        rem_dist = ((anchor - rem_features) ** 2).sum(1).mean()
    else:
        rem_dist = 0.
    loss = (pos_dist - 0.3*(neg_dist + rem_dist) + 1.0).clamp(min=0)
    return loss

def dist_based_logisitc(self, triplet_features, margin):
    """
    distance based logistic loss
    """
    from math import exp
    def logloss(true_label, predicted, eps=exp(-15)):
        p = torch.clamp(predicted, eps, 1 - eps)
        if true_label == 1:
            return -torch.log(p)
        else:
    ...
    if triplet_features.size(0) > 3:
        ...
        rem_dist = 0.
    #pos_dist_prob = (1 + exp(-margin)) / (1 + torch.exp(pos_dist - margin))
    #neg_dist_prob = (1 + exp(-margin)) / (1 + torch.exp(neg_dist - margin))

    #loss = logloss(1, pos_dist_prob) + logloss(0, neg_dist_prob) + logloss(0, rem_dist_prob)
    loss = (pos_dist - 0.3*(neg_dist + rem_dist) + 1.0).clamp(min=0)
    return loss
